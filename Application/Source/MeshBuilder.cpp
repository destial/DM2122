...
Mesh* MeshBuilder::GenerateAxes(const std::string &meshName, float lengthX, float lengthY, float lengthZ) {
	...
	Vertex v;
	v.pos.Set(lengthX, 0.f, 0.f); v.color.Set(1, 0, 0); vertex_buffer_data.push_back(v);
	v.pos.Set(-(lengthX), 0.f, 0.f); v.color.Set(1, 0, 0); vertex_buffer_data.push_back(v);
	v.pos.Set(0.f, lengthY, 0.f); v.color.Set(0, 1, 0); vertex_buffer_data.push_back(v);
	v.pos.Set(0.f, -(lengthY), 0.f); v.color.Set(0, 1, 0); vertex_buffer_data.push_back(v);
	v.pos.Set(0.f, 0.f, lengthZ); v.color.Set(0, 0, 1); vertex_buffer_data.push_back(v);
	v.pos.Set(0.f, 0.f, -(lengthZ)); v.color.Set(0, 0, 1); vertex_buffer_data.push_back(v);
	for (int i = 0; i < 6; i++) {
...
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float lengthX, float lengthY) {
	...
	v.pos.Set(0.5f * lengthX, 0.5f * lengthY, 0.f);	v.color = color; vertex_buffer_data.push_back(v); //v0
	v.pos.Set(-0.5f * lengthX, 0.5f * lengthY, 0.f); v.color = color; vertex_buffer_data.push_back(v); //v1
	v.pos.Set(-0.5f * lengthX, -0.5f * lengthY, 0.f); v.color = color; vertex_buffer_data.push_back(v); //v2
	v.pos.Set(0.5f * lengthX, -0.5f * lengthY, 0.f); v.color = color; vertex_buffer_data.push_back(v); //v3
...
Mesh* MeshBuilder::GenerateSphere(const std::string& meshName, float radius) {
	Vertex v;
	return GenerateSphere(meshName, v.color, 30, 30, radius);
}

Mesh* MeshBuilder::GenerateSphere(const std::string& meshName, Color color, float radius) {
	return GenerateSphere(meshName, color, 30, 30, radius);
}
